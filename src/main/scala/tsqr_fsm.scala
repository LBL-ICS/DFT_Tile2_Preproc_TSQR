...
/////
        ...
        dontTouch(rd_dmx0_en)
        val rd_dmx1_en = Wire(Bool())
        dontTouch(rd_dmx1_en)
        val rd_rtri_en = Wire(Bool())
        dontTouch(rd_rtri_en)
        ...
        when(rst){
            ...
            rd_dmx0_en := (~mx_cnt(0)).asBool & (cnt >= (VK_CY).U & cnt < (VK_CY).U + tr_cy + 1.U).asBool
            rd_dmx1_en := mx_cnt(0) & (cnt >=(VK_CY).U  & cnt <  (VK_CY).U + tr_cy + 1.U)
            rd_rtri_en := (cnt >=(VK_CY).U  & cnt < (VK_CY).U + tr_cy + 1.U)
            dmx0_mem_enb_wire := ((~mx_cnt(0)).asBool & (rd_mem_fst )) | rd_dmx0_en
        ...
        }.elsewhen((~mx_cnt(0)).asBool & (hh_en & (cnt === (VK_CY).U))){
        ...
        when(rst){
            ...
            dmx0_mem_ena_wire := (~mx_cnt(0)).asBool & (tr_cnt >=(TR_CY_MACRO).U ) & (tr_cnt < ((TR_CY_MACRO).U+ tr_cy))
        ...
        when((~mx_cnt(0)).asBool & wr_mem_st){
        ...
        when((~mx_cnt(0)).asBool & (hh_en & (tr_cnt === (TR_CY_MACRO).U))){
...
//switched 3/20
   mem0_fi := (~mx_cnt(0)).asBool & (tr_cnt === (TR_CY_MACRO.U + tr_cy - 1.U)) & (tr_cy === 1.U)
